const loadImagesBtn = () => {
	...
	fetch(
		`http://www.splashbase.co/api/v1/images/search?query=${loadImagesQueryInput}`
	)
		.then((resp) => resp.json())
		.then((result) => {
			...
			cardDiv.innerHTML += result.images
				.map(
					(Data) =>
...
window.onload = (event) => {
	fetch(`http://www.splashbase.co/api/v1/images/search?query=forest`)
		.then((resp) => resp.json())
		.then((result) => {
			const createCarousel = document.getElementById(
				"carouselExampleSlidesOnly"
			)

			for (let i = 0; i < result.images.length; i++) {
				// console.log(result.images[i].url);
				createCarousel.innerHTML = `<div class="carousel-inner">
        ...
        <a class="carousel-control-next" href="#carouselExampleSlidesOnly" role="button" data-slide="next">
          ...
          `
			...
			const filteredResultImages = result.images.filter(
				(value, index, array) => {
					return index < 3
				}
			)
